export interface IGOOD {
  ...
  characters?: GCharacter[];
  artifacts?: GArtifact[];
  weapons?: GWeapon[];
  materials?: Object;
}
export interface GArtifact {
  setKey: GSetKey; //e.g. "GladiatorsFinale"
  slotKey: GSlotKey; //e.g. "plume"
  level: number; //0-20 inclusive
  rarity: number; //1-5 inclusive
  mainStatKey: GStatKey;
  location: GCharacterKey | ''; //where "" means not equipped.
...
export type GSlotKey = 'flower' | 'plume' | 'sands' | 'goblet' | 'circlet';

export interface GWeapon {
  key: GWeaponKey; //"CrescentPike"
